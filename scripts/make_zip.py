...
def  main():
  ...
  installer = "\installer\Orca1 Installer.exe"
   
  if demo:
    installer = "\installer\Orca1 Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Orca1 manual.pdf" 
  ]
  ...
  ZIPNAME = "Orca1-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Orca1-v" + FULLVERSIONSTR + "-win-demo.zip"
